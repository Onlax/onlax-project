                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zONL ought to be enough for anybody." - Bill Gates, 2017
    ui->zONLpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzONLSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzONL->setEnabled(false);
        ui->pushButtonMintzONL->setToolTip(tr("zONL is currently disabled due to maintenance."));

        ui->pushButtonSpendzONL->setEnabled(false);
        ui->pushButtonSpendzONL->setToolTip(tr("zONL is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zONLpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzONL_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zONL is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zONL...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zONL in ") + 
...
void PrivacyDialog::on_pushButtonSpendzONL_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zONL is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zONL
        sendzONL();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zONL
    sendzONL();
}

void PrivacyDialog::on_pushButtonZONLControl_clicked()
{
    ZONLControlDialog* zONLControl = new ZONLControlDialog(this);
    zONLControl->setModel(walletModel);
    zONLControl->exec();
}

void PrivacyDialog::setZONLControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzONLSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzONL()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Onlax Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zONLpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zONL is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ONL </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zONL</b>";
    ...
    // use mints from zONL selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZONLControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZONLControlDialog::GetSelectedMints();
    }
    ...
    // Clear zonl selector in case it was used
    ZONLControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zONL Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " ONL, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zONL Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zONL </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zONL "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zONL "));
    ui->labelzONLAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzONLSyncStatus->setVisible(fShow);
}
