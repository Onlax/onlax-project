...
#include "ui_zonlcontroldialog.h"
...
std::list<std::string> ZONLControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZONLControlDialog::listMints;

ZONLControlDialog::ZONLControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZONLControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZONLControlDialog::~ZONLControlDialog()
{
    delete ui;
}

void ZONLControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZONLControlDialog::updateList()
{
...
void ZONLControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZONLControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZONL_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZONLControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZONLControlDialog::GetSelectedMints()
{
...
void ZONLControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
