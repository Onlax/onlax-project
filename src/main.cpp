...
#if defined(NDEBUG)
#error "Onlax cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zONL spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zONL spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zONL spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 0.12 * COIN;
    } else if (nHeight < 7200 && nHeight >= 480) {
        nSubsidy = 7.8 * COIN;
	} else if (nHeight < 21600 && nHeight >= 7200) {
        nSubsidy = 6 * COIN;
	} else if (nHeight < 43200 && nHeight >= 24600) {
        nSubsidy = 6 * COIN;
	} else if (nHeight < Params().LAST_POW_BLOCK() && nHeight >= 43200) {
        nSubsidy = 6 * COIN;
    } else if (nHeight < 259200 && nHeight >= Params().LAST_POW_BLOCK()) {
        nSubsidy = 6 * COIN;
        ...
        nSubsidy = 4.6 * COIN;
    } else if (nHeight < 907200 && nHeight >= 777600) {
        nSubsidy = 4 * COIN;
    } else if (nHeight < 1036800 && nHeight >= 907200) {
        nSubsidy = 3.3 * COIN;
    } else if (nHeight < 1166400 && nHeight >= 1036800) {
        nSubsidy = 2.5 * COIN;
    } else if (nHeight < 1296000 && nHeight >= 1166400) {
        nSubsidy = 1.6 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight <= 480) {
        ...
        ret = blockValue / (100 / 4);
    } else if (nHeight < 21600 && nHeight >= 7201) {
        ret = blockValue / 5;
	} else if (nHeight < 43200 && nHeight >= 21601) {
        ret = blockValue / (100 / 55);
    } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 43201) {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Onlax
...
{
    RenameThread("onlax-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZONLMinted()
{
...
void RecalculateZONLSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zONL supply
...
bool RecalculateONLSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zONL with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zONLSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ONL to zONL
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zONL spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zONL serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zONL mints and %d zONL spends\n", __func__, nMints, nSpends);
