...
// Onlax: on startup load spork values from previous session if they exist in the sporkDB
...
{
    if (nSporkID == SPORK_11_LOCK_INVALID_UTXO) nValue = SPORK_11_LOCK_INVALID_UTXO_DEFAULT;
    if (nSporkID == SPORK_13_ENABLE_SUPERBLOCKS) nValue = SPORK_13_ENABLE_SUPERBLOCKS_DEFAULT;
    if (nSporkID == SPORK_14_NEW_PROTOCOL_ENFORCEMENT) nValue = SPORK_14_NEW_PROTOCOL_ENFORCEMENT_DEFAULT;
    if (nSporkID == SPORK_15_NEW_PROTOCOL_ENFORCEMENT_2) nValue = SPORK_15_NEW_PROTOCOL_ENFORCEMENT_2_DEFAULT;
    if (nSporkID == SPORK_16_ZEROCOIN_MAINTENANCE_MODE) nValue = SPORK_16_ZEROCOIN_MAINTENANCE_MODE_DEFAULT;
